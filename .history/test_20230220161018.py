...
class url_parser:
    ...
    def __validate_url_with_par(self) -> tuple:
        ...
        try:
            ...
            if not result:
                raise ValueError(
                    f'入参[{self.__url}]不是合法的http url链接')
            
            base_url = result.groups()
            # """返回域名后参数"""
            # novel_arr = self.__url[result.span()[1]:]

            # "无参数，抛出错误"
            # if len(novel_arr) == 0:
            #     raise ValueError(f'[{self.__url}]未携带参数')
            
            # """去除首尾的 '/' """
            # novel_arr = novel_arr[
            #     1 if novel_arr.startswith('/') else 0 :
            #     -1 if novel_arr.endswith('/') else None ]
            
            # novel_arr = novel_arr.split('/')

            # """检查是否所有参数都仅由数字组成"""
            # suffix = '.html'
            # for par in novel_arr:
            #     if par.isdigit():
            #         continue
            #     elif par[:-len(suffix)].isdigit() if par.endswith(suffix) else False:
            #         continue    
            #     raise ValueError(f'[{self.__url}]参数不仅由数字组成')

            logger.info(f'url par = {novel_arr}')
