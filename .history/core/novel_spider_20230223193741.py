...
class novel_spider:
    ...
    def parse_chapter(self, _novel:novel = None) -> novel:
        """从parser中获取章节"""
        try:
            if not self.__url_parser:
                self.__url_parser = url_parser(self.__url)
            ...
            cur_index = 1
            page_url = f'{parser.base_url}{parser.chapter_url}_{cur_index}.html'
            _chapter = chapter(chapter_url)

            logger.info(f'解析章节第{chapter.cur_cnt}页')
            
            while cur_index <= _chapter.page_cnt:
                self.__parse_page(_chapter, page_url)
            
            novel.chapters.appends(chapter)
