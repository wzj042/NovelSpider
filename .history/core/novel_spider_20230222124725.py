...
class novel_spider:
    ...
    def parse_chapter(self, _chapter:chapter = None):
        """从parser中获取章节"""
        try:

            if not _chapter and not _chapter.url:
                raise ValueError('该小说没有章节可以解析')
            
            resp = get(_chapter.url)
            if resp:
                ...
                try:
                    ...
                    """替换图片标签"""
                    chapter_text = text_processor(chapter_text).img_process()
    ...
    def __parse_page(self, chapter:chapter = None) -> str:
        """爬取小说单页内容"""
        try:
            if not chapter:
                raise ValueError('未传入章节实例无法爬取页面')
