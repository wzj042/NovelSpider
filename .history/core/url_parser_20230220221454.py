...
class url_parser:
    ...
    def __parse(self):
        ...
        try:
            ...
            print('novelurl', result.groups())
            if result.group(6):
                """取出章节首页，取出小说页面"""
                self.chapter_url = f'{result.group(4)}.html'
                self.novel_url = result.group(4).split('/')[:2].join('/')
            
            print('novelurl', f'{self.base_url}{self.novel_url}')
            print('chapterurl', f'{self.base_url}{self.chapter_url}')
            print('base_url', f'{self.base_url}')
