...
class novel_spider:
        ...
        try:
            ...
            if resp:
                ...
                try:
                    ...
                    for _chapter in self.get_chapter_list(resp.text):
                        _novel.chapters.append(_chapter)

                    logger.info(f'加载分卷目录1 / {page_count}')
    ...
    def get_chapter_list(self, content:str = '') -> list:
        """
            从页面内容获取章节url
        """
        try:
            ...
            result = pattern.findall(chapter_list_text)
    ...
    def parser_novel(self, _novel:novel = None) -> novel :
        """
            调包返回小说
        """
        try:
            ...
            novel_url = f'{parser.base_url}{parser.novel_url}'
            i = 2
            limit = _novel.page_count
            while i <= limit:
                page_url = f'{novel_url[:-1]}_{i}/'
                logger.info(f'加载分卷目录{i} / {limit}')
                resp = get(page_url)
                for _chapter in self.get_chapter_list(resp.text):
                        _novel.chapters.append(_chapter)
                i += 1
            print(_novel.chapters)
    ...
    def parse_chapter(self, _chapter:chapter = None) -> chapter:
        ...
        try:
            ...
            cur_index = 1
            limit = _chapter.page_cnt
            chapter_url = _chapter.chapter_url
            
            while cur_index <= limit:
                logger.info(f'解析章节第 {cur_index} / {limit} 页')
